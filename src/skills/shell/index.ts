...
export const manifest: SkillManifest = {
  ...
  permissions: [
    { type: 'filesystem-read', description: 'Read command output' },
    { type: 'filesystem-write', description: 'Write artifacts' },
  ],
  tools: [
    {
      name: 'run_command',
      description: 'Execute shell command (dangerous patterns blocked)',
    ...
    },
    {
      ...
      inputSchema: runCommandSchema,
      outputSchema: { type: 'object', properties: { exitCode: { type: 'number' }, summary: { type: 'string' } } },
      riskLevel: 'low',
      requiresApproval: false,
      timeoutMs: 300000,
    },
    {
      ...
      inputSchema: runCommandSchema,
      outputSchema: { type: 'object', properties: { exitCode: { type: 'number' }, durationMs: { type: 'number' } } },
      riskLevel: 'low',
      requiresApproval: false,
      timeoutMs: 600000,
    },
    ...
    for (const pattern of DANGEROUS_PATTERNS) {
      if (normalized.includes(pattern)) {
        return blockedResult(
          'shell',
          toolName,
          `Dangerous command pattern detected: ${pattern}. This requires human approval.`,
          Date.now() - start,
        );
      }
    }
    ...
    if (rawCwd) {
      ...
      if (!rel.startsWith(workspacePath + '/') && !rel.startsWith(workspacePath + '\\') && rel !== workspacePath) {
        return blockedResult(
          'shell',
          toolName,
          `cwd "${rawCwd}" resolves outside workspace — blocked (GOV4-04)`,
          Date.now() - start,
        );
      }
      effectiveCwd = rel;
